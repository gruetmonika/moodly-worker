...
async function poll() {
  try {
    ...
    if (error) { console.error("[poll] RPC error:", error.message); return; }
    if (!data || data.length === 0) { console.log("[poll] Brak oczekujących jobów"); return; }
    ...
    try {
      ...
      await supabase.from("jobs").update({ status: "failed", error: msg }).eq("id", job.id);
    }
  } catch (err) {
